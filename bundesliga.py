...
#   Заранее прошу меня простить, что приходится тратить время на проверку после уловленного дедлайна. Для удобства
# уточню, что изначально использовал немецкую бундеслигу, и все строил на ней, но опустил некоторые правила, до котрых
...
def get_input(filename):  # возможно, данную библеотеку нельзя было использоваться, но из изученных мной
    excel = win32com.client.Dispatch("Excel.Application")  # она показалась самой простой
    filepath = os.getcwd()
